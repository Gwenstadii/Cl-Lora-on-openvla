...
**为什么动作头 A 的漂移致命而 LLM 侧不致命（机制差异）**：
1. **LLM 侧**：specific-LoRA 是**加在巨型预训练残差流上的小修正**（`Wx + s·g·B·A·x`），底层 base 权重冻结且贡献占绝对主导 ⇒ 即使 B_K·A_final 配错，策略只是"略微跑偏"，容错大；且 T=25 的动作 chunk 输出经过多层整流。
