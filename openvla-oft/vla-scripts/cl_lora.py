...
def save_task_bank(model, action_head, bank_dir: str, stage: int) -> None:
    """v13: Save per-task bank — block_scale + action_head only.

    specific LoRA-B is NOT banked — it comes from the latest checkpoint.
    This limits old-task recovery to the capacity of block_scale gating,
    producing the 5-15% retention target of PI's lightweight isolation.
    """
    import os
    ...
    print(f"[TaskBank] v13: Saved stage {stage} bank ({len(bank)} tensors, block_scale + action_head) → {path}")


def load_task_bank(model, action_head, bank_path: str) -> None:
    """v13: Restore block_scale + action_head only (no specific B)."""
