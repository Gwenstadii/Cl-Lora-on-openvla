...
def make_dataset_from_rlds(
    ...
    *,
    ...
    # ==================================================================================  
    TARGET_TASK_INSTRUCTION = "pick up the black bowl next to the cookie box and place it on the plate" 

    if TARGET_TASK_INSTRUCTION is not None:
        print(f"\n[SUCCESS] 匹配到任务: '{TARGET_TASK_INSTRUCTION}'\n")
        def filter_by_instruction(traj):
            instr_seq = traj["task"]["language_instruction"]
            # 比较指令是否匹配 (返回一个布尔数组)
            is_match = tf.equal(instr_seq, tf.constant(TARGET_TASK_INSTRUCTION))
            # 🚨 核心修复：使用 reduce_any 将布尔数组压缩为单个标量 True/False
            return tf.math.reduce_any(is_match)
            
        dataset = dataset.filter(filter_by_instruction)
    # ==================================================================================
