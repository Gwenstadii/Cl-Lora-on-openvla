...
class Attention(nn.Module):  # Attention块(因为调用了lora.Einsum,所以已含lora)
    ...
    @nn.compact
    def __call__(self, xs, positions, attn_mask, kv_cache, freeze_as:Sequence[bool] | None = None, block_scales:Sequence[float] | None = None):  # xs是一个包含多个输入张量的列表，对应多个不同Gemma专家的输入
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):  # 根据每个专家的输入x和配置Config创建对应的QKV计算einsum方式,索引为i
            ...
            if config.num_kv_heads == config.num_heads:
                ...
                qkvs.append(qkv_einsum("BSD,3KDH->3BSKH", x, freeze_a = do_freeze,block_scale = b_scale, 
            ...
            # 第二种情况：KV头数少于Q头数，说明多个Q头共享同一个KV头
                ...
                q = q_einsum("BTD,NDH->BTNH", x, freeze_a = do_freeze, block_scale = b_scale)  # 同理对形状BTD的输入x计算q，得到BTNH形状的q,同理把输出p维度排列好，Batch里每个token的D维映射到N个H维里
                ...
                k, v = kv_einsum("BSD,2KDH->2BSKH", x, freeze_a = do_freeze, block_scale = b_scale)  # 同理对形状BSD的输入x计算k和v，得到2BSKH形状的k和v,同理把输出p维度排列好，Batch里每个token的D维映射到K个H维里（2是因为KV一起算）
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                out.append(out_einsum("BTNH,NHD->BTD", encoded[:, start:end], freeze_a=do_freeze, block_scale=b_scale))  # [:,start:end]等价[:, start:end, :, :],在token维度拆出当前专家提到的特征，并映射回BTD形状并加入输出列表
...
class Block(nn.Module):  # Transformer块
    ...
    @nn.compact
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True, freeze_as_vec=None, block_scales_vec=None):  # noqa: FBT002，deterministic代表是否在推理模式,训练时dropout，推理时不dropout
        ...
        post_attn, kv_cache = attn(pre_attn, positions, attn_mask, kv_cache, freeze_as=freeze_as_vec, block_scales=block_scales_vec)  # 经过Attention块，得到每个专家对应的输出列表和更新后的kv cache
        ...
        for i, (x, config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
                ...
                )(x, freeze_a=curr_freeze, block_scale=curr_scale)  # x经过含lora的前馈网络模块
            out.append(x)
    ...
    def __call__(
        ...
        *,
        ...
        for i, config in enumerate(self.configs):
            ...
            # 默认全 False
            freeze_col = jnp.zeros((depth,), dtype=bool)
            if hasattr(config, "shared_pos") and config.shared_pos:
                # 将 shared_pos 对应的层设为 True
                # 注意 jax 数组更新语法
                idx = jnp.array(config.shared_pos, dtype=jnp.int32)
                freeze_col = freeze_col.at[idx].set(True)
            freeze_cols.append(freeze_col)
        ...
        freeze_stack = jnp.stack(freeze_cols, axis=1)  # [18,2]
        scale_stack = jnp.stack(scale_cols, axis=1)  # [18,2]

        embedded, kv_cache = self.layers(embedded, kv_cache, positions, mask, adarms_cond, deterministic, freeze_stack, scale_stack)  # 调用layers实例，输入是block定义要求的，输出是block里定义的return的xs和kv_cache
