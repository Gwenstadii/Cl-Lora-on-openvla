...
def _build_rehearsal_runtime(config: _config.TrainConfig, data_sharding) -> _RehearsalRuntime | None:
    ...
    if reh.source == "offline_buffer":
        ...
        for buffer_dir in reh.buffer_dirs:
            replay_task_id = int(getattr(config.model, "cl_task_id", 0))
            meta_path = pathlib.Path(buffer_dir) / "meta.json"
            if meta_path.exists():
                import json

                with meta_path.open("r", encoding="utf-8") as f:
                    replay_meta = json.load(f)
                replay_task_id = int(replay_meta.get("cl_task_id", replay_task_id))
            loader = _data_loader.create_replay_buffer_data_loader(
                config,
                replay_buffer_dir=buffer_dir,
                manifest_filename=reh.buffer_manifest_filename,
        ...
        if not hasattr(config.data, "target_task_name"):
            raise TypeError(
                "rehearsal.source='task_dataset' requires a data config with target_task_name. "
                "For RoboTwin/LeRobotAlohaDataConfig, use offline_buffer replay after adding "
                "a RoboTwin-compatible replay buffer loader."
            )
        ...
        for task_name in reh.task_names:
            ...
            task_ids.append(int(getattr(config.model, "cl_task_id", 0)))
            loaders.append(loader)
            iters.append(iter(loader))

    if not iters:
        raise ValueError("No valid rehearsal loaders were created.")
    logging.info("Initialized rehearsal loaders for: %s", list(zip(tasks, task_ids)))
    return _RehearsalRuntime(tasks=tasks, task_ids=task_ids, loaders=loaders, iters=iters)
...
def _next_rehearsal_batch(runtime: _RehearsalRuntime, strategy: str):
    ...
    try:
        return next(runtime.iters[idx]), runtime.task_ids[idx]
...
def _make_cl_lora_weight_decay_mask(trainable_params: nnx.State) -> nnx.State:
    ...
    no_decay_keys = set(trainable_params.filter(nnx_utils.PathRegex(".*lora.*")).flat_state())
    ...
    with jax.default_device(jax.devices("cpu")[0]):
        ...
        if partial_params is not None:
            ...
            train_state = dataclasses.replace(
                ...
                ema_params=None if config.ema_decay is None else student_state,
            )
...
def train_step_cl_lora(
    ...
    def loss_fn(
        ...
        def _flow_task_loss(local_model, local_rng, local_observation, local_actions, local_task_id=None):
            ...
            v_t = local_model.forward_denoise(local_observation, local_actions, noise, time, task_id=local_task_id)
        ...
        if reh_cfg is not None and reh_cfg.enabled:
            ...
            def _compute_replay_loss(_):
                ...
                return _flow_task_loss(model, replay_rng, replay_obs, replay_actions, replay_task_id)
...
def main(config: _config.TrainConfig):
    ...
    # replay_task_id selects a CL-LoRA bank, so it must stay as a Python static
    # value instead of becoming a traced JAX scalar.
    ptrain_steps: dict[int, Any] = {}

    def _get_ptrain_step(static_replay_task_id: int):
        static_replay_task_id = int(static_replay_task_id)
        if static_replay_task_id not in ptrain_steps:
            ptrain_steps[static_replay_task_id] = jax.jit(
                functools.partial(train_step_cl_lora, config, static_replay_task_id),
                in_shardings=(
        ...
        return ptrain_steps[static_replay_task_id]
    ...
    for step in pbar:
        ...
        if replay_enabled:
            replay_batch, replay_task_id = _next_rehearsal_batch(rehearsal_runtime, config.rehearsal.sample_strategy)
        else:
        ...
        with sharding.set_mesh(mesh):
            ptrain_step = _get_ptrain_step(replay_task_id)
        ...
        if step % config.log_interval == 0:
            ...
            # 打印 Loss
            info_str = (
                f"L_total={reduced_info['loss_total']:.4f} "
                f"(Task={reduced_info['loss_task']:.4f}, "
                f"Replay={reduced_info['loss_rehearsal']:.4f}, "
                f"KD={reduced_info['loss_kd']:.4f})"
            )
