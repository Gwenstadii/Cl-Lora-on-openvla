...
class Pi0(_model.BaseModel):
    def __init__(self, config: pi0_config.Pi0Config, rngs: nnx.Rngs):
        ...
        paligemma_config = _gemma.with_cl_lora_options(
            paligemma_config,
            task_bank_size=config.cl_num_tasks,
            use_task_lora_b_bank=config.cl_pali_lora_bank,
            use_task_block_weight_bank=config.cl_pali_block_weight_bank,
            freeze_shared_lora=config.cl_freeze_shared_lora,
            freeze_lora_b=config.cl_freeze_pali_lora_b,
            freeze_lora_a=config.cl_freeze_lora_a,
        )
        action_expert_config = _gemma.with_cl_lora_options(
            action_expert_config,
            task_bank_size=config.cl_num_tasks,
            use_task_lora_b_bank=config.cl_action_lora_bank,
            use_task_block_weight_bank=config.cl_action_block_weight_bank,
            freeze_shared_lora=config.cl_freeze_shared_lora,
            freeze_lora_b=config.cl_freeze_action_lora_b,
            freeze_lora_a=config.cl_freeze_lora_a,
        )
        self.cl_task_id = config.cl_task_id
    ...
    def forward_denoise(
        ...
        *,
        ...
        # 5. Transformer 前向传播
        task_id = self.cl_task_id if task_id is None else task_id
    ...
    def extract_behavior_features(
        ...
        *,
        ...
        """Extract deterministic context and action-token features for offline coreset selection.

        This follows the same multimodal transformer path as ``forward_denoise``
        and selects the requested CL-LoRA task bank. It intentionally returns
        latent features only and does not alter training or policy inference.
        """
        ...
        prefix_weights = prefix_mask.astype(jnp.float32)[..., None]
        context_feature = jnp.sum(prefix_out.astype(jnp.float32) * prefix_weights, axis=1)
        context_feature /= jnp.maximum(jnp.sum(prefix_weights, axis=1), 1.0)
        action_features = suffix_out[:, -self.action_horizon :].astype(jnp.float32)
        return context_feature, action_features
