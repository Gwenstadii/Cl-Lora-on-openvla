...
class LoRAConfig:  # Lora adapter的配置(att和ffn里的都是这个配置类）)
    ...
    label: str = "L"
    # CL-LoRA v2: optionally select a task-specific LoRA-B/up projection
    # while keeping the LoRA-A/down projection shared.
    ...
    def setup(self):
        ...
        if config := self.lora_config:
            ...
            if config.use_orthogonal_init:
                ...
                b_init = config.init_fn
            if config.use_task_lora_b_bank:
                self.w_b_bank = self.param("lora_b_bank", b_init, (config.task_bank_size, *shape_b))
            else:
                self.w_b_bank = None
            ###end3
    ...
    def __call__(
        ...
        *,
        ...
        if config := self.lora_config:
            ...
            if config.use_task_lora_b_bank and task_id is not None:
                task_w_b = jnp.take(self.w_b_bank.astype(dtype), jnp.asarray(task_id, dtype=jnp.int32), axis=0)
                w_b = jnp.where(jnp.asarray(use_task_bank, dtype=jnp.bool_), task_w_b, w_b)
            # 使用 jnp.where 替代 if，完美兼容 JAX Tracer 并且能正确阻断梯度
            freeze_a = jnp.logical_or(
                jnp.asarray(freeze_a, dtype=jnp.bool_),
                jnp.asarray(config.freeze_lora_a, dtype=jnp.bool_),
            )
            w_a = jnp.where(freeze_a, jax.lax.stop_gradient(w_a), w_a)
            w_b = jnp.where(jnp.asarray(freeze_b, dtype=jnp.bool_), jax.lax.stop_gradient(w_b), w_b)
            ###end4
...
class FeedForward(nn.Module):  # 前馈网络的LoRA实现
    ...
    def setup(self):
        ...
        self.w_gating_lora_b_bank = None
        self.w_linear_lora_b_bank = None
        ...
        if config := self.lora_config:
            ...
            if config.use_task_lora_b_bank:
                self.w_gating_lora_b_bank = self.param(
                    "gating_einsum_lora_b_bank",
                    get_init_fn(False),
                    (config.task_bank_size, 2, self.lora_config.rank, self.hidden_dim),
                )
                self.w_linear_lora_b_bank = self.param(
                    "linear_lora_b_bank",
                    get_init_fn(False),
                    (config.task_bank_size, self.lora_config.rank, self.features),
                )
            ###end7
    ...
    def __call__(
        ...
        *,
        ...
        use_task_bank: bool = False,
        freeze_b: bool = False,
    ):  # x的形状是（batch_size， seq_len, features）
        dtype = x.dtype  # original dtype, could be half-precision
        ...
        ff_gate = self._dot(
            ...
            if self.w_gating_lora is None
                ...
                self.w_gating_lora[0][0],
                self.w_gating_lora[1][0],
                None if self.w_gating_lora_b_bank is None else self.w_gating_lora_b_bank[:, 0],
            ),
            freeze_a,
            block_scale,
            task_id=task_id,
        ...
        ff1 = self._dot(
            ...
            if self.w_gating_lora is None
                ...
                self.w_gating_lora[0][1],
                self.w_gating_lora[1][1],
                None if self.w_gating_lora_b_bank is None else self.w_gating_lora_b_bank[:, 1],
            ),
            freeze_a,
            block_scale,
            task_id=task_id,
        ...
        outputs = self._dot(
            ...
            else (self.w_linear_lora[0], self.w_linear_lora[1], self.w_linear_lora_b_bank),
            freeze_a,
            block_scale,
            task_id=task_id,
            use_task_bank=use_task_bank,
    ...
    def _dot(
        ...
        x: at.Array,
        w: at.Array,
        lora_weights: tuple[at.Array, ...] | None,
        freeze_a: bool = False,
        block_scale: float | None = None,
        *,
        task_id=None,
        ...
        if len(lora_weights) > 2 and lora_weights[2] is not None and task_id is not None:
            task_w_b = jnp.take(lora_weights[2].astype(x.dtype), jnp.asarray(task_id, dtype=jnp.int32), axis=0)
            w_b = jnp.where(jnp.asarray(use_task_bank, dtype=jnp.bool_), task_w_b, w_b)
        ...
        freeze_a = jnp.logical_or(
            jnp.asarray(freeze_a, dtype=jnp.bool_),
            jnp.asarray(self.lora_config.freeze_lora_a, dtype=jnp.bool_),
        )
        w_a = jnp.where(freeze_a, jax.lax.stop_gradient(w_a), w_a)
        w_b = jnp.where(jnp.asarray(freeze_b, dtype=jnp.bool_), jax.lax.stop_gradient(w_b), w_b)
